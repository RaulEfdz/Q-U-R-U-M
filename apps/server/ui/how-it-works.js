...
function diagramaConexionOffline() {
  return lienzo(1160, 330,
    ...
    caja(520, 96, 190, 94, 'RED DISPONIBLE', { detalle: 'Internet · VPN · LAN' }),
    caja(760, 96, 170, 94, 'P2P + NOISE', { detalle: 'DHT · canal cifrado' }),
    caja(980, 96, 160, 94, 'SERVIDOR CENTRAL', { detalle: 'laptop o Azure', clase: 'e-ok' }),
...
export function pintarComoFunciona(seccion) {
  ...
  pintar(seccion,
    ...
    bloque('Conexión offline: cuándo viaja la información',
      h('p', { clase: 'nota', texto: 'El celular funciona como una libreta inteligente: entiende la visita y conserva la evidencia aunque no haya señal. El servidor entra después, cuando existe una conexión disponible. Offline describe la captura y la inferencia local; la sincronización ocurre cuando aparece una red.' }),
      conDiagrama(diagramaConexionOffline()),
      h('p', { clase: 'nota', texto: 'La red disponible puede ser Internet pública con descubrimiento DHT, una VPN entre dispositivos o una LAN controlada. Por eso un trabajador en Panamá puede sincronizar con un servidor central en otra ciudad o en Azure. La red solo transporta el lote: no ejecuta la IA ni convierte el servidor en una API pública.' }),
      h('p', { clase: 'nota', texto: 'El peer central no está atado a una computadora específica: puede ejecutarse en una laptop para la demo, en una VM de Azure, Google Compute Engine, AWS EC2 u otro host persistente con Node 22. El despliegue necesita un proceso de larga duración, acceso de red para Hyperswarm, almacenamiento persistente para data/ y las claves autorizadas en QUORUM_PEERS. La arquitectura es compatible con la nube; una instancia de Azure o Google todavía requiere una prueba operativa propia antes de presentarla como validada.' }),
      ...
      h('ol', { clase: 'lista-explica' },
        ...
        h('li', { texto: 'El servidor web sigue escuchando solo en 127.0.0.1:3000 en la configuración actual. El celular no entra por HTTP: se encuentra con Hyperswarm en el topic P2P. En un despliegue remoto, Azure sería el peer central, no una API HTTP abierta.' }),
        h('li', { texto: 'La clave pública del celular debe estar en QUORUM_PEERS. Una allowlist vacía rechaza todos los peers por deny-by-default.' }),
        h('li', { texto: 'El servidor responde un ACK por observación: recibida, duplicada o rechazada. Sin ACK, el móvil conserva el dato y lo vuelve a intentar.' }),
        h('li', { texto: 'La versión actual ya implementa móvil, ACK incremental, reintentos e idempotencia. La evidencia recibida queda en Revisión P2P hasta que una persona la confirma.' }),
        h('li', { texto: 'El flujo remoto trabajador → servidor central funciona igual si ese peer vive en una laptop, Azure, Google Cloud, AWS u otro host persistente. La compatibilidad está en el protocolo P2P; la evidencia comprobada hoy es Pixel 8 Pro ↔ servidor local, no una instancia cloud.' }))),
